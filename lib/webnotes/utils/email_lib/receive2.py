...
class IMAPMailbox:
	...
	def get_userinfo(self):
		# pwd=webnotes.conn.sql("""select password from __Auth where user='pranali.k@indictranstech.com'""")
		
		print "in the get_userinfo"
		for user in webnotes.conn.sql("""select name,response, sync_email from `tabProfile` 
				 where name <>'Administrator' and name <> 'Guest' and response is not null and name like '%pranali%'""",as_dict=1,debug=1):
			print user['name']

			print user['response']
			print user['sync_email']
	...
	def get_sent_messages(self,name):
		...
		print name
		print self.get_sent_email_uid()[:100]
	...
	def get_sent_email_uid(self):
		...
		status, msgs = self.imap.select('"[Gmail]/Sent Mail"')
		print "status"
		print status
