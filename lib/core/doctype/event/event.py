...
def sync_google_event(_type='Post'):
	...
	while True:
		...
		for event in events['items']:
			...
			# webnotes.errprint(event['start']['dateTime'])
			# webnotes.errprint(event['end']['dateTime'])
			if a not in eventlist:
				...
				webnotes.conn.sql("update `tabEvent` set starts_on=%s, ends_on=%s,subject=%s where event_id=%s",(event['start']['dateTime'],event['end']['dateTime'],event['summary'],event['id']),debug=1)
